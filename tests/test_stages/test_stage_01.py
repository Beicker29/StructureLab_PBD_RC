...
from structurelab_pbd_rc.design.stages.stage_01_material_characterization import run


def test_stage_01_prepares_output_directories(tmp_path: Path) -> None:
    result = run(
        config_path=Path("configs/stages/stage_01_material_characterization.yaml"),
...
def test_stage_01_writes_initial_results_json(tmp_path: Path) -> None:
    result = run(
        config_path=Path("configs/stages/stage_01_material_characterization.yaml"),
        output_root=tmp_path,
    )
    ...
    assert results_path == tmp_path / "stage_01" / "data" / "stage_01_results.json"

    payload = json.loads(results_path.read_text(encoding="utf-8"))
    assert payload["stage_id"] == "stage_01"
    assert payload["status"] == "completed"
    assert payload["computed_results"]["concrete_curves"]
    assert payload["metrics"]


def test_stage_01_creates_expected_artifacts(tmp_path: Path) -> None:
    result = run(
        config_path=Path("configs/stages/stage_01_material_characterization.yaml"),
        output_root=tmp_path,
    )
    ...
    assert (
        ...
        == tmp_path / "stage_01" / "data" / "models" / "mander_classic_unconfined_concrete"
    )
    assert "report_pdf" not in result["generated_files"]
    assert "report_calculated_parameters_yaml" not in result["generated_files"]
    assert not (tmp_path / "stage_01" / "reports" / "documento").exists()
    assert not (tmp_path / "stage_01" / "reports" / "models").exists()


def test_stage_01_reports_model_parameters_without_global_report(tmp_path: Path) -> None:
    result = run(
        config_path=Path("configs/stages/stage_01_material_characterization.yaml"),
        output_root=tmp_path,
    )
    ...
    assert not (tmp_path / "stage_01" / "reports" / "stage_01_calculated_parameters.yaml").exists()
    assert not (tmp_path / "stage_01" / "reports" / "stage_01_report.pdf").exists()


def test_stage_01_writes_one_yaml_per_constitutive_model(tmp_path: Path) -> None:
    result = run(
        config_path=Path("configs/stages/stage_01_material_characterization.yaml"),
        output_root=tmp_path,
    )
