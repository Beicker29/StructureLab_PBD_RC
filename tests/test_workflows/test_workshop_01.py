...
from structurelab_pbd_rc.design.workshops.workshop_01_material_characterization import run
...
def test_workshop_01_creates_expected_artifacts(tmp_path: Path) -> None:
    ...
    assert (
        ...
        == tmp_path / "workshop_01" / "data" / "models" / "mander_classic_unconfined_concrete"
    )
    assert "report_pdf" not in result["generated_files"]
    assert "report_calculated_parameters_yaml" not in result["generated_files"]
    assert not (tmp_path / "workshop_01" / "reports" / "documento").exists()
    assert not (tmp_path / "workshop_01" / "reports" / "models").exists()


def test_workshop_01_reports_model_parameters_without_global_report(tmp_path: Path) -> None:
    result = run(
        config_path=Path("configs/workshops/workshop_01_material_characterization.yaml"),
        output_root=tmp_path,
    )
    ...
    assert not (tmp_path / "workshop_01" / "reports" / "workshop_01_calculated_parameters.yaml").exists()
    assert not (tmp_path / "workshop_01" / "reports" / "workshop_01_report.pdf").exists()


def test_workshop_01_writes_one_yaml_per_constitutive_model(tmp_path: Path) -> None:
    result = run(
        config_path=Path("configs/workshops/workshop_01_material_characterization.yaml"),
        output_root=tmp_path,
    )
