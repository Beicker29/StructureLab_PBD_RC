...
from structurelab_pbd_rc.geometry.confined_core import ConfinedCore
...
def calculate_rectangular_confinement_parameters(
    ...
    *,
    ...
    wi_x_cm = geometry.core_width_cm / spaces_per_side
    wi_y_cm = geometry.core_height_cm / spaces_per_side
    sum_wi2_cm2 = 2.0 * spaces_per_side * wi_x_cm**2 + 2.0 * spaces_per_side * wi_y_cm**2
    ...
    fl_eff_mpa = 0.5 * ke * rho_s * transverse_yield_strength_mpa

    assumptions = [
        "Core dimensions are measured to the tie centerline unless YAML states otherwise.",
        "wi spacing is inferred from a symmetric perimeter layout with configured bars per side.",
