"""Flujo de Etapa 2: caracterizacion de materiales."""
...
DEFAULT_CONFIG_PATH = Path("configs/stage_02/material_characterization.yaml")
...
def validate_stage_02_config(config: dict[str, Any]) -> None:
    """Validate the minimum editable structure required by Etapa 2."""

    require_keys(config, REQUIRED_TOP_LEVEL_KEYS, context="stage_02 configuration")
...
def build_mander_unconfined_memory_lines(model_report: dict[str, Any]) -> list[str]:
    ...
    lines.extend(
        [
            ...
            "- YAML del modelo: outputs/stage_02/reports/mander_classic_unconfined_concrete/mander_classic_unconfined_concrete.yaml",
            "- Figura del modelo: outputs/stage_02/figures/models/mander_classic_unconfined_concrete.png",
            "- Datos CSV/XLSX: outputs/stage_02/data/models/mander_classic_unconfined_concrete/",
        ]
    )
    return lines
...
def _mander_unconfined_input_rows(inputs_used: dict[str, object]) -> str:
    ...
    rows = (
        (
            "Resistencia maxima del concreto no confinado",
            "$f'_{co}$",
            f"{_format_number(float(inputs_used['f_co']))} [MPa]",
        ),
        (
            "Expresion del esfuerzo ultimo a traccion",
            "$f_t$",
            "$0.62\\sqrt{f'_{co}}$",
        ),
        ("Modulo elastico usado en traccion", "$E_t$", "$E_t = E_c$"),
        (
            "Expresion de la deformacion de traccion",
            "$\\varepsilon_t$",
            "$\\varepsilon_t = f_t/E_t$",
        ),
        (
            "Deformacion asociada a la resistencia maxima",
            "$\\varepsilon_{co}$",
            f"{_format_number(float(inputs_used['epsilon_co']))} [mm/mm]",
        ),
        (
            "Deformacion de descascaramiento",
            "$\\varepsilon_{sp}$",
...
def _mander_unconfined_parameter_rows(parameters: dict[str, dict[str, object]]) -> str:
    ...
    rows = (
        ("Resistencia maxima del concreto no confinado", "$f'_{co}$", "f_co", "-"),
        ("Modulo elastico del concreto", "$E_c$", "Ec", "$4700\\sqrt{f'_{co}}$"),
        ("Esfuerzo ultimo a traccion del concreto", "$f_t$", "ft", "$0.62\\sqrt{f'_{co}}$"),
        ("Modulo elastico en traccion", "$E_t$", "Et", "$E_t = E_c$"),
        ("Deformacion de traccion", "$\\varepsilon_t$", "epsilon_t", "$f_t/E_t$"),
        ("Deformacion en la resistencia maxima", "$\\varepsilon_{co}$", "epsilon_co", "-"),
        ("Limite de la rama curva de compresion", "$2\\varepsilon_{co}$", "epsilon_2co", "$2\\varepsilon_{co}$"),
        ("Deformacion de descascaramiento", "$\\varepsilon_{sp}$", "epsilon_sp", "-"),
        ("Modulo secante", "$E_{sec}$", "Esec", "$f'_{co}/\\varepsilon_{co}$"),
        ("Parametro de forma de Mander", "$r$", "r", "$E_c/(E_c-E_{sec})$"),
    )
    return "\n".join(
...
def build_mander_unconfined_quarto_document(model_report: dict[str, Any]) -> str:
    ...
    stage_title = "Caracterizacion mecanica monotonica de materiales"
...
![Curva esfuerzo-deformacion del modelo de Mander clasico no confinado.]({figure_path}){{width=100%}}
...
def build_calculated_parameters_report(
    ...
    *,
    ...
    return {
        ...
        "title": config["title"],
        "source_inputs": "configs/stage_02/material_characterization.yaml",
        "units": config["units"],
        "input_data": {
                "title": "Datos de entrada",
                "description": "Datos leidos desde el YAML de la Etapa 2 antes de calcular parametros derivados.",
                "data": {
                    "section": {
...
def build_initial_results(
    ...
    *,
    ...
    """Build the Etapa 2 result dictionary."""
...
def run(
    ...
    """Read Etapa 2 configuration and prepare output directories.
    ...
    validate_stage_02_config(result["config"])
    ...
    generated_paths["figure_concrete"] = plot_stress_strain_curves(
        ...
        xlabel="Deformacion unitaria, epsilon_c [mm/mm]",
    ...
    generated_paths["figure_steel_tension"] = plot_stress_strain_curves(
        ...
        xlabel="Deformacion unitaria de traccion, epsilon_s [mm/mm]",
        ylabel="Esfuerzo de traccion, fs [MPa]",
    )
    generated_paths["figure_steel_buckling"] = plot_stress_strain_curves(
        ...
        subtitle="Comparacion con y sin degradacion por pandeo | flejes #4 @ 100 [mm]",
        xlabel="Deformacion unitaria de compresion, epsilon_s [mm/mm]",
        ylabel="Esfuerzo de compresion, fs [MPa]",
    )
    generated_paths["figure_mesh"] = plot_stress_strain_curves(
        mesh_curves,
        figures_dir / "welded_wire_mesh.png",
        title="Malla electrosoldada",
        subtitle=f"Diametro seleccionado = {config['welded_wire_mesh']['default_diameter']} [mm]",
        xlabel="Deformacion unitaria de traccion, epsilon_s [mm/mm]",
        ylabel="Esfuerzo de traccion, fs [MPa]",
    )
    generated_paths["figure_core_sketch"] = plot_confined_core_sketch(
        ...
        title="Seccion base de la Etapa 2",
    )
    ...
    for old_global_report in (
        reports_dir / "stage_02_calculated_parameters.yaml",
        reports_dir / "stage_02_report.pdf",
    ):
    ...
    results_path = stage_results_json_path(output_dirs, filename="stage_02_results.json")
