...
from structurelab_pbd_rc.mechanics.materials.concrete.equations import (
    mander_classic_confined_strength,
    mander_classic_ultimate_strain,
    mander_peak_strain,
    mander_r_parameter,
    secant_modulus,
)
...
class ManderClassicConcreteModel:
    ...
    def summary_parameters(self) -> dict[str, float]:
        ...
        fcc = mander_classic_confined_strength(f_co=fc, fl=fl_eff)
        fcc = max(fcc, fc)
        eps_cc = mander_peak_strain(epsilon_co=self.parameters.epsilon_co, fcc=fcc, f_co=fc)
        eps_cu = mander_classic_ultimate_strain(
            rho_s=rho_s,
            fyh=fyh,
            epsilon_su=self.parameters.transverse_steel_ultimate_strain,
            fcc=fcc,
        )
    ...
    def generate_curve(self, num_points: int = 401) -> dict[str, object]:
        ...
        return {
            "name": "mander_classic_confined_concrete",
            "strain": strains,
            "stress": stresses,
            "parameters": self.summary_parameters(),
            "warnings": [],
